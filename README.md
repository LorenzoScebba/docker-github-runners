...
| **`snapd` purged**                              | Not just defensive: `snapd` is a Go binary that carries ~165 distinct CVEs (16 Critical — the same stale-Go stdlib + vendored `x/crypto` cluster the `gosu` shim removes). Purging it is the single biggest CVE reduction after `gosu`.                                                                                                                                                                                                       |
| **Chrome via Chrome for Testing, not snap**     | Ubuntu's apt `chromium` is a snap stub that pulls in `snapd` (see above) and won't even run once snap is stripped. We drop [Chrome for Testing](https://googlechromelabs.github.io/chrome-for-testing/) instead — a versioned zip of the same Chromium, but a C++ binary grype finds nothing in, so it adds **zero** distinct CVEs. Cypress auto-detects it as `chrome-for-testing`. `linux64` only, so it's guarded to the amd64 image.       |
